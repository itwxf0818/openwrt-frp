# Derived from ImmortalWrt packages commit 8509f551edb7beb4a6324afca4d84b2bea404b66.
# See docs/SOURCES.md for provenance and local adaptations.
# shellcheck disable=SC3043,SC2015,SC2018,SC2019,SC2129,SC2317,SC2329
# Keep upstream ASCII boolean folding and redirection style.
...
_emit_common() {
	...
	# TLS
	# Preserve the older official UCI name when no master value is set.
	local tls_force
	config_get tls_force "$section" tls_force
	[ -n "$tls_force" ] || config_get tls_force "$section" tls_only
	_toml_line transport.tls.force "$tls_force" bool
...
_watch_conf_file() {
	...
	[ -r "$file" ] && procd_append_param file "$file"
	return 0
}
...
frp_master_start() {
	...
	# Validate the generated TOML with the same template environment.
	if ! (
		set -- verify -c "$CONF_FILE"
		[ "$_ALLOW_UNSAFE_TOKEN_SOURCE_EXEC" = 0 ] || set -- "$@" --allow-unsafe=TokenSourceExec
	...
	procd_append_param file "/etc/config/$NAME"

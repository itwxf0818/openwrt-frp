...
用于编译固件时更新内置的 `frpc` / `frps`，自动跟踪 [FRP 官方稳定版](https://github.com/fatedier/frp/releases/latest)。当前版本见 [Makefile](Makefile)。
...
每天检查上游稳定版，更新版本和源码校验值，测试及 SDK 构建全部通过后自动提交。失败时保留原版本。

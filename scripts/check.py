...
for service in ("frpc", "frps"):
    with (ROOT / "files" / f"{service}.toml").open("rb") as f:
        data = tomllib.load(f)
    assert data["auth"]["method"] == "token"
    assert data["log"]["to"] == "console"
    config = (ROOT / "files" / f"{service}.config").read_text()
    assert "option enabled '0'" in config
for path in [ROOT / "Makefile", * (ROOT / "files").iterdir(), *(ROOT / "scripts").glob("*.sh")]:
    raw = path.read_bytes()
